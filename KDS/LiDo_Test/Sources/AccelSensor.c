...
#endif
#if PL_CONFIG_HAS_ACCEL_ISR1_PIN && PL_CONFIG_HAS_SHELL
  #include "ACC_INT1.h"
#endif
...
uint8_t AccelSensor_GetTemperatureSensorEnabled(bool *enabled) {
  ...
  res = GI2C1_ReadByteAddress8(ACCEL_SENS_I2C_ADDRESS, ACCEL_SENS_I2C_REGISTER_TEMP_CFG, &data);
  *enabled = data==0xC0;
  return res;
}

uint8_t AccelSensor_EnableTemperatureSensor(void) {
  return GI2C1_WriteByteAddress8(ACCEL_SENS_I2C_ADDRESS, ACCEL_SENS_I2C_REGISTER_TEMP_CFG, 0xC0);
}

uint8_t AccelSensor_DisableTemperatureSensor(void) {
  return GI2C1_WriteByteAddress8(ACCEL_SENS_I2C_ADDRESS, ACCEL_SENS_I2C_REGISTER_TEMP_CFG, 0x00);
}

uint8_t AccelSensor_WhoAmI(uint8_t *id) {
  /* should be 0b00110011 (0x33) */
  return GI2C1_ReadByteAddress8(ACCEL_SENS_I2C_ADDRESS, ACCEL_SENS_I2C_REGISTER_WHOAMI, id);
}

uint8_t AccelSensor_SetPowerMode(LIS2DH_CTRL_REG1_PowerMode mode) {
  uint8_t val, res;

  res = GI2C1_ReadByteAddress8(ACCEL_SENS_I2C_ADDRESS, ACCEL_SENS_I2C_REGISTER_CTRL_REG1, &val);
  if (res!=ERR_OK) {
    return res;
  }
...
void AccelSensor_init(void) {
	...
	res |= AccelSensor_EnableTemperatureSensor();		//Enable Temp. Measurement
	if(res != ERR_OK) {
...
static uint8_t PrintStatus(CLS1_ConstStdIOType *io) {
  ...
  res = AccelSensor_WhoAmI(&id);
  if (res==ERR_OK) {
    UTIL1_strcpy(buf, sizeof(buf), (unsigned char*)"0x");
    UTIL1_strcatNum8Hex(buf, sizeof(buf), id);
    UTIL1_strcat(buf, sizeof(buf), (unsigned char*)"\r\n");
  } else {
    UTIL1_strcpy(buf, sizeof(buf), (unsigned char*)"ERROR\r\n");
  }
  CLS1_SendStatusStr((unsigned char*)"  WhoAmI", buf, io->stdOut);

  res = AccelSensor_GetTemperatureSensorEnabled(&enabled);
  if (res==ERR_OK) {
    CLS1_SendStatusStr((unsigned char*)"  Temp enabled", enabled?(unsigned char*)"yes\r\n":(unsigned char*)"no\r\n", io->stdOut);
  } else {
    CLS1_SendStatusStr((unsigned char*)"  Temp enabled", (unsigned char*)"ERROR\r\n", io->stdOut);
  }
  ...
  res = AccelSensor_getValues(&values);
  if (res==ERR_OK) {
